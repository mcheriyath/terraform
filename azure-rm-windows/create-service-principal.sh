...
DEFAULT_IDENTIFIER=SERVICEPRINCIPALv`date +"%Y%m%d%M%S"`
IDENTIFIER="1"
...
CLIENT_ID=`azure ad app create --name "$IDNAME" --password "$CLIENT_SECRET" --identifier-uris ""$IDURIS"" --home-page ""$HOMEPAGE"" | grep  "AppId:" | awk -F':' '{ print $3 } ' | tr -d ' '`
